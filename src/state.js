...
export const STORAGE_KEY = "bitacora.personal.state.v1";
...
export function completeDiscovery(state, consentedAt) {
  ...
  next.activityEvents.unshift({
    ...
    source: "manual",
    type: "PROFILE",
    title: "Perfil inicial confirmado",
    summary: `${next.profile.displayName} revisó y aprobó los datos guardados por Conocerme.`,
...
export function upsertAlarm(state, input) {
  ...
  next.activityEvents.unshift({
    ...
    source: "manual",
    type: index >= 0 ? "ALARM_UPDATED" : "ALARM_CREATED",
    title: alarm.title,
    summary: index >= 0 ? "Alarma actualizada." : "Alarma creada.",
    sourceUrl: "local://alarms",
    occurredAt: now
  });
  return saveState(next);
}
...
export function upsertEvent(state, input) {
  ...
  next.activityEvents.unshift({
    ...
    source: "manual",
    type: index >= 0 ? "EVENT_UPDATED" : "EVENT_CREATED",
    title: event.title,
    summary: index >= 0 ? "Evento actualizado." : "Evento creado.",
    sourceUrl: "local://calendar",
    occurredAt: now
  });
  return saveState(next);
}
...
export function addLifeCheckin(state, input) {
  ...
  const checkin = {
    ...
    movement: Number(input.movement),
    discipline: Number(input.discipline),
    learning: Number(input.learning),
    personalPriority: input.personalPriority || "",
    note: input.note || ""
  };
  ...
  next.activityEvents.unshift({
    ...
    source: "manual",
    type: "LIFE_CHECKIN",
    title: "Check-in de vida registrado",
    summary: `Energía ${checkin.energy}/10 · Foco ${checkin.focus}/10`,
    sourceUrl: "local://life",
    occurredAt: now
  });
  return saveState(next);
}
...
export function addQuickCapture(state, input) {
  ...
  if (capture.needsOwner) {
    next.attentionItems.unshift({
      ...
      reason: "Marcaste esta captura como una dependencia real.",
